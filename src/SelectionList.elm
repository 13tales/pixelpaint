...
map f list =
    { list
        | previous = List.map f list.previous
...
map2 f listA listB =
    { listA
        | previous = List.map2 f listA.previous listB.previous
...
indexedMap f list =
    let
        ...
        nextLength =
            List.length list.next
    in
        { list
            | previous = List.reverse (List.indexedMap f (List.reverse list.previous))
...
selectedMap f list =
    { list
        | previous = List.map (f False) list.previous
...
{-| Go to the next item in the selection list, looping around to the first item once we reach the end of the list.
-}
cycleSelection : SelectionList a -> SelectionList a
cycleSelection list =
    let
        reachedLastItem =
            if selectedIndex list == (length list - 1) then
                True
            else
                False
    in
        case reachedLastItem of
            True ->
                goto 0 list
